...
import cache, client,logging,requests
...
def request(url, post=None, headers=None, mobile=False, safe=False, timeout=30):
    #try:
        ...
        try:
          ...
          tokens, user_agent = cache.get(cfscrape.get_tokens,3,domain,timeout,headers, table='cookies')
        ...
        if 'jschl-answer' in result.content:
            ...
            tokens, user_agent = cache.get(cfscrape.get_tokens,0,domain,timeout,headers, table='cookies')
            if headers==None:
              headers = {'User-Agent': user_agent}
