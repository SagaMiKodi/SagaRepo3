#url2='http://stream.moviegrabber.tv/resolve/'+url
#-*- coding: utf-8 -*-
import re,urllib2, base64, math,logging
...
def getMediaLinkForGuest(url):
    ...
    sHtmlContent1 = requests.get(url).content
    #logging.warning(sHtmlContent1)
    ...
    logging.warning(url)
    regex='<p style="" class="" id="(.+?)"'
    match=re.compile(regex).findall(sHtmlContent1)
    if (aResult):
        TabUrl = aResult
    else:
        logging.warning('OPL er 1')
        return False,False
    ...
    for i in TabUrl:
        if len(i[1]) > 30:
            Coded_url = i[1]
            Item_url = '#' + i[0]
            logging.warning( Item_url + ' : ' + Coded_url )

    if not(Coded_url):
        logging.warning('Url codee non trouvee')
        return False, False
    ...
    logging.warning('Code JS extrait')
    ...
    logging.warning(JP.HackVars)
    url = None
    for name in [ '#streamurl', '#streamuri', '#streamurj' ,'#'+match[0]]:
        if JP.IsVar( JP.HackVars, name ):
            url = JP.GetVarHack( name )
            logging.warning( 'Decoded url ' + name + ' : ' + url )
            break
